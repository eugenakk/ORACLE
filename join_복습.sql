...
select decode(grouping_id(p.category),0,p.category,'총주문량') 제품분류
count(*) 주문량
from products p, order_items oi
where p.product_id=oi.product_id
group by p.category;
...
desc product s;
insert into products values(600,'새제품','아이스크림','메이커:', 3000);
insert into products values(610,'새제품','아이스크림','메이커:', 3000);
commit;
