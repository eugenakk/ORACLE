...
-----------------------------------기존의 데이터를 건드리지 않고 테스트 할떄, 따로 같은 테이블을 copy해서 테스팅한다. 
drop table emp2;
create table emp2
as                        -- as 뒤에 서브쿼리를 넣었다 .. 이것은 COPY table과 같은 역할을 한다****
select * from emp;
...
-- truncate table명
-->테이블에 있는 전체 데이터를 삭제해준다. (delete from table 이랑 같은 역할을 한다)
--->차이점이 있긴 함: rollback을 이용해 복구가 안된다. 
